...
try{
 ...
 const rooms=[];
 for(const id of process.argv.includes('--kitchen-only')?['kitchen']:['livingroom','kitchen']){
  if(!process.argv.includes('--render-only')){if(id==='livingroom')throw Error('Livingroom geometry was rejected. Rebuild it before exporting; use --kitchen-only for the corrected kitchen.');
 ...
 if(rooms.length){if(process.argv.includes('--kitchen-only')){const old=JSON.parse(await fs.readFile(path.join(root,'house/house.json'),'utf8'));for(const r of old.rooms)if(!rooms.some(x=>x.id===r.id)){r.status='rejected';rooms.push(r);}}const source=JSON.parse(await fs.readFile(path.join(root,'house/source-house-map.json'),'utf8'));const manifest={schemaVersion:1,status:'Previous pair rejected; kitchen rebuilt for review; livingroom still rejected',units:'meters',up:'+Y',forward:'+Z into room from main doorway',avatarHeightMeters:1.166,scaleBasis:'Estimated household proportions scaled by 1.166/1.75; no measured survey available',source:'https://github.com/gloriaariayvette-lgtm/Vintos-main/blob/main/scripts/house-map.json',portalContract:{trigger:'Transparent box with alpha=0; use geometry bounds for collision',spawn:'Use target portal spawn and inwardNormal, with re-entry cooldown until clear of trigger',pending:'Do not activate pending-review, rejected-partner, or any transition involving an unapproved room. Reciprocal node names are reserved for later room files.',rotation:'Room-local +Z faces inward. Rotate avatar to target inwardNormal; do not copy source-room coordinates.'},rooms,plannedRooms:source.rooms.filter(r=>!rooms.some(b=>b.id===r.id)).map(r=>({id:r.id.replace('cats room','catsroom'),status:'not-built',photographed:!['laundry','hall','bathroom','closet','stairs','cats room'].includes(r.id),adjacent:r.adjacent.map(x=>x.replace('cats room','catsroom'))}))};await fs.writeFile(path.join(root,'house/house.json'),JSON.stringify(manifest,null,2)+'\n');}
